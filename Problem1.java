...
class Solution {
    ...
    //we will take candidates as paramater, just one path not a new arraylist each time since we will be back tracking,
    // index to keep track of choose and no choose and target
    private void backtrack(int[] candidates, List<Integer> path, int index, int target){
        ...
        if(target == 0){
            //you want to create a new array list here and add it to the result because we are back tracking and removind end elements
            // from the path array so create a copy and add it to the final result array.
