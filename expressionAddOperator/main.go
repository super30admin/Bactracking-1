/*
    ...
    - How do we maintain and enforce PEMDAS operator precedence?
        ...
        - Thats what the tail parameter is for
            - We need to look back and undo when running into a multiplication
            - The tail acts as the previous thing we did
            - Undo tail by removing tail from calc value ( calc - tail )
            - Then first multiply tail(previous number) with current because multiplecation takes precedence ( tail * curr )
            - Finally add our result back; cacl-tail + (tail * curr)
